...
public class BoardInteraction extends SimpleBlockInteraction {
  ...
  protected void interactWithBlock(@NonNull World world, @NonNull CommandBuffer<EntityStore> commandBuffer,
    ...
    player.getPageManager().openCustomPage(ref, ref.getStore(),
        new BoardGameUi(player.getPlayerRef(), CustomPageLifetime.CanDismiss));
  ...
  private void activateBoardCamera(@NonNull Player player, @NonNull Vector3i boardPosition) {
    ...
    LOGGER.atInfo().log("Board camera activated for player %s at position (%f, %f, %f)", player.getDisplayName(),
